...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        input_tensor = input_tensor.to(self.conv.weight.device)
    ...
    def init_hidden(self, batch_size):
        return (Variable(torch.zeros(batch_size, self.hidden_dim, self.height, self.width).type_as(self.conv.weight).to(self.conv.weight.device)),
                Variable(torch.zeros(batch_size, self.hidden_dim, self.height, self.width).type_as(self.conv.weight)).to(self.conv.weight.device))
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        assert len(input_tensor.shape) == 5, f"{str(input_tensor.shape)}"
